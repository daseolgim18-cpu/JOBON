...
document.addEventListener("click", (e) => {
  ...
  if (remove) {
    const row = remove.closest(".nested-row,.trouble-row");
    const box = row?.parentElement;
    if (row && box && box.children.length > 1) row.remove();
    else if (row) row.querySelectorAll("input,textarea").forEach((x) => (x.value = ""));
    return;
  }
  const add = e.target.closest("[data-add-row]");
  if (!add) return;
  const box = document.getElementById(add.dataset.addRow);
